...
- Стек: **Next.js** (App Router), **TypeScript**, **shadcn/ui**, **Tailwind CSS**, **pnpm** (`frontend/`; дорожная карта работ — [tasks/tasklist-frontend.md](tasks/tasklist-frontend.md), продуктовый план — [plan.md](plan.md)).
...
| **Ученик** | Пользователь с ролью ученика; привязан к Telegram; в профиле — класс, телефон, email (опционально) |
| **Тема** | Единица учебного содержания (модуль курса) |
| **Занятие** | Конкретная сессия: дата, тема, длительность (по умолчанию 60 мин), статус, что принести |
...
- Сбои LLM и отказ провайдера (включая неверный ключ в конфигурации) не должны раскрывать секреты в логах и ответах клиентам; пользователю — обобщённое сообщение (например временная недоступность ассистента).
