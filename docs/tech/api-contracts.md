...
| `notes` | `string` \| `null` | Заметки / цель обучения (поле добавляется в схему БД в итерации backend) |

`telegram_id` в теле **не передаётся** с формы преподавателя.
...
- `POST /v1/students` — при конфликте уникальности (например e-mail уже занят) — `409` `conflict` (или узкоспециализированный `code`).
